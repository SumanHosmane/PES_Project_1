...
{	
	...
	{	
		...
		{	
			...
			printf("How many values do you want to enter: ");				//Number of values user wants to add
			count = takeinput();

			if(offset + count > total_mem)							//offset check for out of bounds
			{
				printf("You are writing outside allocated space. Try again.\n");
				return 0;
			}

			else if(count==0)								//nothing to be read error
			{
				printf("0 is an invalid input. Try again.\n");
				count = takeinput();
			}

			for(int i=0;i<count;i++)							//taking values from user
			{
			...
			memptr=(memptr-count);
			for(int i=0;i<count;i++)							//echoes back written values for user reference
			{
