...
{			
	...
	else
	{
		...
		if((inputs <= 5) && ((offset+inputs)<=total_mem))	//generation of random values without rand()
		{
			...
			clock_t t;
			t= clock();
			num[0]=((((seed*9)/2)+7)*2)%17;			//Equations to generate
			num[1]=(((seed/2)*10)%11);			//pseudo random numbers
			num[2]=(((876*seed)/4)%12)+3;			//These are self made
			num[3]=(((seed+4)*6)-5)%13;			//equations
